function [st,t,f] = stockwell(timeseries,minfreq,maxfreq,samplingrate,freqsamplingrate)
...
% verbose = TRUE; changed by marzieh      
verbose = FALSE; 
removeedge= FALSE;
...
spe_nelements =ceil((maxfreq - minfreq+1)/freqsamplingrate); % by marzieh
% spe_nelements =ceil(50*(maxfreq - minfreq+length(timeseries))/length(timeseries));   % by marzieh
% spe_nelements =50;   % by marzieh
f = (minfreq + [0:spe_nelements-1]*freqsamplingrate)/(samplingrate*length(timeseries));
% f = (minfreq + ([0:spe_nelements-1]*maxfreq)/(spe_nelements))/(length(timeseries));  % by marzieh
